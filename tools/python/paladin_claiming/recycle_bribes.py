...
def claim_paladin_bribes(claims: List[dict], chain_name: str) -> str:
    ...
    with open(csv_path, "w") as f:
        ...
        for claim in claims:
            ...
            f.write(f"{chain_name},{claim['token']},{claim['gauge']},{amount},{claim['amount']}\n")
...
def deposit_hh_bribes(csv_path: str, chain_name: str, builder: SafeTxBuilder):
    ...
    with open(csv_path, "r") as f:
        ...
        for line in f:
            ...
            key = (token, gauge)
            aggregated_bribes[key] = aggregated_bribes.get(key, 0) + int(amount_mantissa)
    ...
    for (token, gauge), amount_mantissa in aggregated_bribes.items():
        ...
        # sell 30% later
        token_sell_amounts[token] = token_sell_amounts.get(token, 0) + amount_mantissa - bribe_amount_mantissa
    ...
    builder.output_payload(payload_chain_dir / f"paladin_bribe_recycling_{chain_name}_{CURRENT_DATE}.json")
