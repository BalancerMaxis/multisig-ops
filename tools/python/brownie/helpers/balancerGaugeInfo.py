...
def buildGaugesDictFromContract():
    ...
    while i < n:
        ...
        if (gauge._name =="ArbitrumRootGauge"):
            ...
            pool = w3arbitrum.eth.contract(address=pool, abi=json.load(open("web3py/abi/IRewardsOnlyGauge.json")))
            ...
            crosschain_streamer = w3polygon.eth.contract(address=gauge.getRecipient(),
                                                         abi=json.load(open("web3py/abi/IChildChainStreamer.json")))
            pool = crosschain_streamer.functions.reward_receiver().call()
            pool = w3polygon.eth.contract(address=pool, abi=json.load(open("web3py/abi/IRewardsOnlyGauge.json")))
            ...
            crosschain_streamer = w3optimism.eth.contract(address=gauge.getRecipient(),
                                                         abi=json.load(open("web3py/abi/IChildChainStreamer.json")))
            pool = crosschain_streamer.functions.reward_receiver().call()
            pool = w3optimism.eth.contract(address=pool, abi=json.load(open("web3py/abi/IRewardsOnlyGauge.json")))
