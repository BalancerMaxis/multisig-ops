...
class Balancer:
    ...
    def get_amount_out(self, asset_in, asset_out, amount_in, pool=None):
        ...
        swap = [(pool_id, asset_in_index, asset_out_index, amount_in, b"")]
        ...
        amounts = self.vault.queryBatchSwap.call(
            calc_out_given_in, swap, underlyings, funds
        )
    ...
    def get_pool_data(self, update_cache=False):
        ...
        if update_cache:
            ...
            pool_data_filtered = [
                x
                for x in pool_data
                if Decimal(x["totalLiquidity"]) > self.pool_query_liquidity_threshold
            ]
    ...
    def _deposit_and_stake(self, pool, request, stake=True, destination=None):
    ...
    def unstake_all_and_withdraw_all(
        ...
        request = (underlyings, min_underlyings_out, data_encoded, is_eth)

        self._unstake_and_withdraw_all(pool, request, unstake, claim, destination)
    ...
    def unstake_and_withdraw_all_single_asset(
        ...
        data_encoded = eth_abi.encode_abi(
            ["uint256", "uint256", "uint256"], [0, amount_in, underlying_index]
        )
    ...
    def _unstake_and_withdraw_all(self, pool, request, unstake, claim, destination):
        destination = self.safe if not destination else destination
        pool_id = pool.getPoolId()
        ...
        self.vault.exitPool(pool_id, self.safe, destination, request)
    ...
    def claim_all(self, underlyings=None, pool=None):
        ...
        balances_before = [
            Contract(gauge.reward_tokens(x)).balanceOf(self.safe)
            for x in range(reward_count)
        ]
    ...
    def swap(self, asset_in, asset_out, mantissa_in, pool=None, destination=None):
        ...
        min_out = self.get_amount_out(asset_in, asset_out, mantissa_in, pool=pool) * (
            1 - self.max_slippage
        )
    ...
    def lock_bal(
        self, mantissa_bal=None, mantissa_weth=None, mantisssa_bpt=None, lock_days=365
    ):
        """
        ...
        after_bpt = (
            (bpt.balanceOf(self.safe) - before_bpt) * self.dusty
            if depositing
            else mantisssa_bpt
        )
