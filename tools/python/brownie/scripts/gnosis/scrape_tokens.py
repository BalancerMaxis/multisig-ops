...
def scrape(address, receiver=""):
    with Progress() as progress:
        ...
        for token_addr in token_set:
            if token_addr in [
                registry.eth.rari.dai_manager,
                registry.eth.rari.unitroller,
            ]:
                continue
    ...
    console.print(table)
    pd.DataFrame(token_data).to_csv(
        f"{os.getcwd()}/scripts/gnosis/{address}_token_balances.csv", index=False
    )
