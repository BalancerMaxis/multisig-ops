...
def main():
    ...
    sweeps = {}
    report = ""
    with open(f"FeeData/{today}.json", "r") as f:
        data = json.load(f)
    if chain.id == 1:
        sweep_limit = 10000
    else:
    ...
    # Generate JSON
    with open("FeeData/feeSweep.json", "r") as f:
    ...
    with open(f"FeeData/out/{today}.json", "w") as f:
        json.dump(dict(tx_out_map), f)
    with open(f"FeeData/out/{today}.report.txt", "w") as f:
        f.write(report)
