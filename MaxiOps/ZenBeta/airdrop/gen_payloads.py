...
def to_wei(amount):
    rounded = Decimal(amount).to_integral_value(rounding=ROUND_DOWN)
    return int(rounded * Decimal('1e18'))


airdrop = pd.read_csv("ZenBetaAirdrop.csv").dropna(subset=["Address"])
mainnet_recipients = airdrop[(airdrop["Pay on mainnet"] == "Yes") & (airdrop["Total"] > 0)]
gnosis_recipients = airdrop[(airdrop["Pay on mainnet"] != "Yes") & (airdrop["Total"] > 0)]
bridge_to_gnosis = to_wei(gnosis_recipients["Total"].sum())


### mainnet transactions
mainnet_builder = SafeTxBuilder(
...
omnibridge.relayTokens(
    bal.address, AddrBook("gnosis").multisigs.lm, bridge_to_gnosis
)
...
### gnosis transactions
gnosis_builder = SafeTxBuilder(
    "multisigs/lm", chain_name="gnosis"
)
